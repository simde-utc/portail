...
@section('content')
	<div style="position: absolute; width: 100%; top: 50%; transform: translateY(-50%); display: flex; justify-content: center;">
		<div>
			...
			<br />
			<input type="button" id="connectCAS" value="Se connecter" onClick="window.location.href = '{{ config('portail.cas.url').'login?service='.route('login.process', ['provider' => $provider, 'redirect' => $redirect]) }}'"></input>
		</div>
	</div>
@endsection
