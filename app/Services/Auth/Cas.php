...
class Cas extends Auth
{
	...
	protected static $model = 'AuthCas';
	...
	public static function authenticate($service, $ticket) {
		...
		$userAuth = self::findUser(self::$model, 'login', $userArray['cas:user']);

		if ($userAuth === null) {
			// Si inconnu, on le crée et on le connecte.
			self::create(self::$model, $userArray['cas:attributes']['cas:mail'], $userArray['cas:attributes']['cas:givenName'], $userArray['cas:attributes']['cas:sn'], [
			...
			self::update(self::$model, $userAuth->user_id, $userArray['cas:attributes']['cas:givenName'], $userArray['cas:attributes']['cas:sn'], [
				'login' => $userArray['cas:user'],
				'email' => $userArray['cas:attributes']['cas:mail'],
			]);
		}
